...
typedef TRI_REAL **triangle;            /* Really:  typedef triangle *triangle   */
...
typedef TRI_REAL **subseg;                  /* Really:  typedef subseg *subseg   */
...
/* The vertex data structure.  Each vertex is actually an array of TRI_REALs.    */
/*   The number of TRI_REALs is unknown until runtime.  An integer boundary      */
/*   marker, and sometimes a pointer to a triangle, is appended after the    */
/*   TRI_REALs.                                                                  */

typedef TRI_REAL *vertex;
...
struct badtriang {
  ...
  TRI_REAL key;                             /* cos^2 of smallest (apical) angle. */
...
struct event {
  TRI_REAL xkey, ykey;                              /* Coordinates of the event. */
  TRI_VOID *eventptr;      /* Can be a vertex or the location of a circle event. */
...
struct memorypool {
  TRI_VOID **firstblock, **nowblock;
  TRI_VOID *nextitem;
  TRI_VOID *deaditemstack;
  TRI_VOID **pathblock;
  TRI_VOID *pathitem;
  int alignbytes;
  int itembytes;
...
TRI_REAL splitter;       /* Used to split TRI_REAL factors for exact multiplication. */
TRI_REAL epsilon;                             /* Floating-point machine epsilon. */
TRI_REAL resulterrbound;
TRI_REAL ccwerrboundA, ccwerrboundB, ccwerrboundC;
TRI_REAL iccerrboundA, iccerrboundB, iccerrboundC;
TRI_REAL o3derrboundA, o3derrboundB, o3derrboundC;
...
/* Other variables. */
  ...
  TRI_REAL xmin, xmax, ymin, ymax;                            /* x and y bounds. */
  TRI_REAL xminextreme;      /* Nonexistent x value used as a flag in sweepline. */
...
/* Read the instructions to find out the meaning of these switches.          */
  ...
  int nobisect;
  int steiner;
  TRI_REAL minangle, goodangle, offconstant;
  TRI_REAL maxarea;
...
#define elemattribute(otri, attnum)                                           \
  ((TRI_REAL *) (otri).tri)[m->elemattribindex + (attnum)]

#define setelemattribute(otri, attnum, value)                                 \
  ((TRI_REAL *) (otri).tri)[m->elemattribindex + (attnum)] = value

/* Check or set a triangle's maximum area bound.                             */

#define areabound(otri)  ((TRI_REAL *) (otri).tri)[m->areaboundindex]

#define setareabound(otri, value)                                             \
  ((TRI_REAL *) (otri).tri)[m->areaboundindex] = value
...
int triunsuitable(vertex triorg, vertex tridest, vertex triapex, TRI_REAL area)
...
TRI_REAL area;                                      /* The area of the triangle. */
#endif /* not ANSI_DECLARATORS */

{
  TRI_REAL dxoa, dxda, dxod;
  TRI_REAL dyoa, dyda, dyod;
  TRI_REAL oalen, dalen, odlen;
  TRI_REAL maxlen;
...
#ifdef ANSI_DECLARATORS
TRI_VOID *trimalloc(int size)
#else /* not ANSI_DECLARATORS */
TRI_VOID *trimalloc(size)
int size;
#endif /* not ANSI_DECLARATORS */

{
  ...
  memptr = (TRI_VOID *) malloc((unsigned int) size);
  if (memptr == (TRI_VOID *) NULL) {
      ...
      for (j = STARTINDEX; argv[i][j] != '\0'; j++) {
        ...
        if (argv[i][j] == 'q') {
          ...
          if (((argv[i][j + 1] >= '0') && (argv[i][j + 1] <= '9')) ||
            ...
            workstring[k] = '\0';
            b->minangle = (TRI_REAL) strtod(workstring, (char **) NULL);
	  } else {
            b->minangle = 20.0;
	  }
	}
        if (argv[i][j] == 'a') {
          ...
          if (((argv[i][j + 1] >= '0') && (argv[i][j + 1] <= '9')) ||
            ...
            workstring[k] = '\0';
            b->maxarea = (TRI_REAL) strtod(workstring, (char **) NULL);
            if (b->maxarea <= 0.0) {
...
{
  pool->firstblock = (TRI_VOID **) NULL;
  pool->nowblock = (TRI_VOID **) NULL;
  pool->nextitem = (TRI_VOID *) NULL;
  pool->deaditemstack = (TRI_VOID *) NULL;
  pool->pathblock = (TRI_VOID **) NULL;
  pool->pathitem = (TRI_VOID *) NULL;
  pool->alignbytes = 0;
  pool->itembytes = 0;
...

{
  ...
  /* Find the first item in the pool.  Increment by the size of (TRI_VOID *). */
  alignptr = (unsigned long) (pool->nowblock + 1);
  /* Align the item on an `alignbytes'-byte boundary. */
  pool->nextitem = (TRI_VOID *)
...

{
  ...
  /*   - sizeof(TRI_VOID *), so the stack of dead items can be maintained */
  /*       without unaligned accesses.                                */
  if (alignment > sizeof(TRI_VOID *)) {
    pool->alignbytes = alignment;
  } else {
    pool->alignbytes = sizeof(TRI_VOID *);
  }
  ...
  pool->firstblock = (TRI_VOID **)
    trimalloc(pool->itemsfirstblock * pool->itembytes + (int) sizeof(TRI_VOID *) +
              pool->alignbytes);
...
{
  while (pool->firstblock != (TRI_VOID **) NULL) {
    pool->nowblock = (TRI_VOID **) *(pool->firstblock);
    trifree((TRI_VOID *) pool->firstblock);
...
#ifdef ANSI_DECLARATORS
TRI_VOID *poolalloc(struct memorypool *pool)
#else /* not ANSI_DECLARATORS */
TRI_VOID *poolalloc(pool)
struct memorypool *pool;
#endif /* not ANSI_DECLARATORS */

{
  ...
  if (pool->deaditemstack != (TRI_VOID *) NULL) {
    newitem = pool->deaditemstack;               /* Take first item in list. */
    pool->deaditemstack = * (TRI_VOID **) pool->deaditemstack;
  } else {
    /* Check if there are any free items left in the current block. */
    if (pool->unallocateditems == 0) {
      /* Check if another block must be allocated. */
      if (*(pool->nowblock) == (TRI_VOID *) NULL) {
        /* Allocate a new block of items, pointed to by the previous block. */
        newblock = (TRI_VOID **) trimalloc(pool->itemsperblock * pool->itembytes +
                                       (int) sizeof(TRI_VOID *) +
                                       pool->alignbytes);
        *(pool->nowblock) = (TRI_VOID *) newblock;
        /* The next block pointer is NULL. */
        *newblock = (TRI_VOID *) NULL;
      }

      /* Move to the new block. */
      pool->nowblock = (TRI_VOID **) *(pool->nowblock);
      /* Find the first item in the block.    */
      /*   Increment by the size of (TRI_VOID *). */
      alignptr = (unsigned long) (pool->nowblock + 1);
      /* Align the item on an `alignbytes'-byte boundary. */
      pool->nextitem = (TRI_VOID *)
        (alignptr + (unsigned long) pool->alignbytes -
    ...
    pool->nextitem = (TRI_VOID *) ((char *) pool->nextitem + pool->itembytes);
...
#ifdef ANSI_DECLARATORS
void pooldealloc(struct memorypool *pool, TRI_VOID *dyingitem)
#else /* not ANSI_DECLARATORS */
...

{
  ...
  *((TRI_VOID **) dyingitem) = pool->deaditemstack;
...

{
  ...
  /* Find the first item in the block.  Increment by the size of (TRI_VOID *). */
  alignptr = (unsigned long) (pool->pathblock + 1);
  /* Align with item on an `alignbytes'-byte boundary. */
  pool->pathitem = (TRI_VOID *)
    (alignptr + (unsigned long) pool->alignbytes -
     (alignptr % (unsigned long) pool->alignbytes));
...
#ifdef ANSI_DECLARATORS
TRI_VOID *traverse(struct memorypool *pool)
#else /* not ANSI_DECLARATORS */
TRI_VOID *traverse(pool)
struct memorypool *pool;
#endif /* not ANSI_DECLARATORS */

{
  ...
  if (pool->pathitem == pool->nextitem) {
    return (TRI_VOID *) NULL;
  }

  /* Check whether any untraversed items remain in the current block. */
  if (pool->pathitemsleft == 0) {
    /* Find the next block. */
    pool->pathblock = (TRI_VOID **) *(pool->pathblock);
    /* Find the first item in the block.  Increment by the size of (TRI_VOID *). */
    alignptr = (unsigned long) (pool->pathblock + 1);
    /* Align with item on an `alignbytes'-byte boundary. */
    pool->pathitem = (TRI_VOID *)
  ...
  pool->pathitem = (TRI_VOID *) ((char *) pool->pathitem + pool->itembytes);
  pool->pathitemsleft--;
  return newitem;
}
...

{
  ...
  m->vertexmarkindex = ((m->mesh_dim + m->nextras) * sizeof(TRI_REAL) +
                        sizeof(int) - 1) /
                       sizeof(int);
...

{
  ...
  /*   where the index is measured in TRI_REALs.                           */
  m->elemattribindex = (trisize + sizeof(TRI_REAL) - 1) / sizeof(TRI_REAL);
  /* The index within each triangle at which the maximum area constraint  */
  /*   is found, where the index is measured in TRI_REALs.  Note that if the  */
  /*   `regionattrib' flag is set, an additional attribute will be added. */
  m->areaboundindex = m->elemattribindex + m->eextras + b->regionattrib;
  /* If triangle attributes or an area bound are needed, increase the number */
  /*   of bytes occupied by a triangle.                                      */
  if (b->vararea) {
    trisize = (m->areaboundindex + 1) * sizeof(TRI_REAL);
  } else if (m->eextras + b->regionattrib > 0) {
    trisize = m->areaboundindex * sizeof(TRI_REAL);
  }
...

{
  ...
  pooldealloc(&m->triangles, (TRI_VOID *) dyingtriangle);
}
...

{
  ...
  pooldealloc(&m->subsegs, (TRI_VOID *) dyingsubseg);
}
...

{
  ...
  pooldealloc(&m->vertices, (TRI_VOID *) dyingvertex);
}
...

{
  ...
  pooldealloc(&m->badsubsegs, (TRI_VOID *) dyingseg);
}
...

{
  ...
  trifree((TRI_VOID *) m->dummytribase);
  if (b->usesegments) {
    pooldeinit(&m->subsegs);
    trifree((TRI_VOID *) m->dummysubbase);
  }
...
#ifdef ANSI_DECLARATORS
int fast_expansion_sum_zeroelim(int elen, TRI_REAL *e, int flen, TRI_REAL *f, TRI_REAL *h)
#else /* not ANSI_DECLARATORS */
...
{
  TRI_REAL Q;
  INEXACT TRI_REAL Qnew;
  INEXACT TRI_REAL hh;
  INEXACT TRI_REAL bvirt;
  TRI_REAL avirt, bround, around;
...
#ifdef ANSI_DECLARATORS
int scale_expansion_zeroelim(int elen, TRI_REAL *e, TRI_REAL b, TRI_REAL *h)
#else /* not ANSI_DECLARATORS */
...
{
  INEXACT TRI_REAL Q, sum;
  TRI_REAL hh;
  INEXACT TRI_REAL product1;
  TRI_REAL product0;
  int eindex, hindex;
  TRI_REAL enow;
  INEXACT TRI_REAL bvirt;
  TRI_REAL avirt, bround, around;
  INEXACT TRI_REAL c;
  INEXACT TRI_REAL abig;
  TRI_REAL ahi, alo, bhi, blo;
  TRI_REAL err1, err2, err3;
...
#ifdef ANSI_DECLARATORS
TRI_REAL estimate(int elen, TRI_REAL *e)
#else /* not ANSI_DECLARATORS */
TRI_REAL estimate(elen, e)
int elen;
TRI_REAL *e;
#endif /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
TRI_REAL counterclockwiseadapt(vertex pa, vertex pb, vertex pc, TRI_REAL detsum)
#else /* not ANSI_DECLARATORS */
TRI_REAL counterclockwiseadapt(pa, pb, pc, detsum)
vertex pa;
vertex pb;
vertex pc;
TRI_REAL detsum;
...
{
  INEXACT TRI_REAL acx, acy, bcx, bcy;
  TRI_REAL acxtail, acytail, bcxtail, bcytail;
  INEXACT TRI_REAL detleft, detright;
  TRI_REAL detlefttail, detrighttail;
  TRI_REAL det, errbound;
  TRI_REAL B[4], C1[8], C2[12], D[16];
  INEXACT TRI_REAL B3;
  int C1length, C2length, Dlength;
  TRI_REAL u[4];
  INEXACT TRI_REAL u3;
  INEXACT TRI_REAL s1, t1;
  TRI_REAL s0, t0;
  ...
  TRI_REAL ahi, alo, bhi, blo;
  TRI_REAL err1, err2, err3;
  INEXACT TRI_REAL _i, _j;
  TRI_REAL _0;

  acx = (TRI_REAL) (pa[0] - pc[0]);
  bcx = (TRI_REAL) (pb[0] - pc[0]);
  acy = (TRI_REAL) (pa[1] - pc[1]);
  bcy = (TRI_REAL) (pb[1] - pc[1]);
...
#ifdef ANSI_DECLARATORS
TRI_REAL counterclockwise(struct mesh *m, struct behavior *b,
                      vertex pa, vertex pb, vertex pc)
#else /* not ANSI_DECLARATORS */
TRI_REAL counterclockwise(m, b, pa, pb, pc)
struct mesh *m;
struct behavior *b;
vertex pa;
vertex pb;
...
{
  TRI_REAL detleft, detright, det;
  TRI_REAL detsum, errbound;
...
#ifdef ANSI_DECLARATORS
TRI_REAL incircleadapt(vertex pa, vertex pb, vertex pc, vertex pd, TRI_REAL permanent)
#else /* not ANSI_DECLARATORS */
TRI_REAL incircleadapt(pa, pb, pc, pd, permanent)
vertex pa;
vertex pb;
vertex pc;
vertex pd;
...
{
  INEXACT TRI_REAL adx, bdx, cdx, ady, bdy, cdy;
  TRI_REAL det, errbound;

  INEXACT TRI_REAL bdxcdy1, cdxbdy1, cdxady1, adxcdy1, adxbdy1, bdxady1;
  TRI_REAL bdxcdy0, cdxbdy0, cdxady0, adxcdy0, adxbdy0, bdxady0;
  TRI_REAL bc[4], ca[4], ab[4];
  INEXACT TRI_REAL bc3, ca3, ab3;
  TRI_REAL axbc[8], axxbc[16], aybc[8], ayybc[16], adet[32];
  int axbclen, axxbclen, aybclen, ayybclen, alen;
  TRI_REAL bxca[8], bxxca[16], byca[8], byyca[16], bdet[32];
  int bxcalen, bxxcalen, bycalen, byycalen, blen;
  TRI_REAL cxab[8], cxxab[16], cyab[8], cyyab[16], cdet[32];
  int cxablen, cxxablen, cyablen, cyyablen, clen;
  TRI_REAL abdet[64];
  int ablen;
  TRI_REAL fin1[1152], fin2[1152];
  TRI_REAL *finnow, *finother, *finswap;
  int finlength;

  TRI_REAL adxtail, bdxtail, cdxtail, adytail, bdytail, cdytail;
  INEXACT TRI_REAL adxadx1, adyady1, bdxbdx1, bdybdy1, cdxcdx1, cdycdy1;
  TRI_REAL adxadx0, adyady0, bdxbdx0, bdybdy0, cdxcdx0, cdycdy0;
  TRI_REAL aa[4], bb[4], cc[4];
  INEXACT TRI_REAL aa3, bb3, cc3;
  INEXACT TRI_REAL ti1, tj1;
  TRI_REAL ti0, tj0;
  TRI_REAL u[4], v[4];
  INEXACT TRI_REAL u3, v3;
  TRI_REAL temp8[8], temp16a[16], temp16b[16], temp16c[16];
  TRI_REAL temp32a[32], temp32b[32], temp48[48], temp64[64];
  int temp8len, temp16alen, temp16blen, temp16clen;
  int temp32alen, temp32blen, temp48len, temp64len;
  TRI_REAL axtbb[8], axtcc[8], aytbb[8], aytcc[8];
  int axtbblen, axtcclen, aytbblen, aytcclen;
  TRI_REAL bxtaa[8], bxtcc[8], bytaa[8], bytcc[8];
  int bxtaalen, bxtcclen, bytaalen, bytcclen;
  TRI_REAL cxtaa[8], cxtbb[8], cytaa[8], cytbb[8];
  int cxtaalen, cxtbblen, cytaalen, cytbblen;
  TRI_REAL axtbc[8], aytbc[8], bxtca[8], bytca[8], cxtab[8], cytab[8];
  int axtbclen, aytbclen, bxtcalen, bytcalen, cxtablen, cytablen;
  TRI_REAL axtbct[16], aytbct[16], bxtcat[16], bytcat[16], cxtabt[16], cytabt[16];
  int axtbctlen, aytbctlen, bxtcatlen, bytcatlen, cxtabtlen, cytabtlen;
  TRI_REAL axtbctt[8], aytbctt[8], bxtcatt[8];
  TRI_REAL bytcatt[8], cxtabtt[8], cytabtt[8];
  int axtbcttlen, aytbcttlen, bxtcattlen, bytcattlen, cxtabttlen, cytabttlen;
  TRI_REAL abt[8], bct[8], cat[8];
  int abtlen, bctlen, catlen;
  TRI_REAL abtt[4], bctt[4], catt[4];
  int abttlen, bcttlen, cattlen;
  INEXACT TRI_REAL abtt3, bctt3, catt3;
  TRI_REAL negate;
  ...
  adx = (TRI_REAL) (pa[0] - pd[0]);
  bdx = (TRI_REAL) (pb[0] - pd[0]);
  cdx = (TRI_REAL) (pc[0] - pd[0]);
  ady = (TRI_REAL) (pa[1] - pd[1]);
  bdy = (TRI_REAL) (pb[1] - pd[1]);
  cdy = (TRI_REAL) (pc[1] - pd[1]);
...
#ifdef ANSI_DECLARATORS
TRI_REAL incircle(struct mesh *m, struct behavior *b,
              vertex pa, vertex pb, vertex pc, vertex pd)
#else /* not ANSI_DECLARATORS */
TRI_REAL incircle(m, b, pa, pb, pc, pd)
struct mesh *m;
struct behavior *b;
vertex pa;
vertex pb;
...
{
  TRI_REAL adx, bdx, cdx, ady, bdy, cdy;
  TRI_REAL bdxcdy, cdxbdy, cdxady, adxcdy, adxbdy, bdxady;
  TRI_REAL alift, blift, clift;
  TRI_REAL det;
  TRI_REAL permanent, errbound;
...
#ifdef ANSI_DECLARATORS
TRI_REAL orient3dadapt(vertex pa, vertex pb, vertex pc, vertex pd,
                   TRI_REAL aheight, TRI_REAL bheight, TRI_REAL cheight, TRI_REAL dheight,
                   TRI_REAL permanent)
#else /* not ANSI_DECLARATORS */
TRI_REAL orient3dadapt(pa, pb, pc, pd,
...
{
  INEXACT TRI_REAL adx, bdx, cdx, ady, bdy, cdy, adheight, bdheight, cdheight;
  TRI_REAL det, errbound;
  ...
  TRI_REAL bc[4], ca[4], ab[4];
  INEXACT TRI_REAL bc3, ca3, ab3;
  TRI_REAL adet[8], bdet[8], cdet[8];
  int alen, blen, clen;
  TRI_REAL abdet[16];
  int ablen;
  TRI_REAL *finnow, *finother, *finswap;
  TRI_REAL fin1[192], fin2[192];
  int finlength;

  TRI_REAL adxtail, bdxtail, cdxtail;
  TRI_REAL adytail, bdytail, cdytail;
  TRI_REAL adheighttail, bdheighttail, cdheighttail;
  INEXACT TRI_REAL at_blarge, at_clarge;
  INEXACT TRI_REAL bt_clarge, bt_alarge;
  INEXACT TRI_REAL ct_alarge, ct_blarge;
  TRI_REAL at_b[4], at_c[4], bt_c[4], bt_a[4], ct_a[4], ct_b[4];
  int at_blen, at_clen, bt_clen, bt_alen, ct_alen, ct_blen;
  INEXACT TRI_REAL bdxt_cdy1, cdxt_bdy1, cdxt_ady1;
  INEXACT TRI_REAL adxt_cdy1, adxt_bdy1, bdxt_ady1;
  TRI_REAL bdxt_cdy0, cdxt_bdy0, cdxt_ady0;
  TRI_REAL adxt_cdy0, adxt_bdy0, bdxt_ady0;
  INEXACT TRI_REAL bdyt_cdx1, cdyt_bdx1, cdyt_adx1;
  INEXACT TRI_REAL adyt_cdx1, adyt_bdx1, bdyt_adx1;
  TRI_REAL bdyt_cdx0, cdyt_bdx0, cdyt_adx0;
  TRI_REAL adyt_cdx0, adyt_bdx0, bdyt_adx0;
  TRI_REAL bct[8], cat[8], abt[8];
  int bctlen, catlen, abtlen;
  INEXACT TRI_REAL bdxt_cdyt1, cdxt_bdyt1, cdxt_adyt1;
  INEXACT TRI_REAL adxt_cdyt1, adxt_bdyt1, bdxt_adyt1;
  TRI_REAL bdxt_cdyt0, cdxt_bdyt0, cdxt_adyt0;
  TRI_REAL adxt_cdyt0, adxt_bdyt0, bdxt_adyt0;
  TRI_REAL u[4], v[12], w[16];
  INEXACT TRI_REAL u3;
  int vlength, wlength;
  TRI_REAL negate;
  ...
  bdy = (TRI_REAL) (pb[1] - pd[1]);
  cdy = (TRI_REAL) (pc[1] - pd[1]);
  adheight = (TRI_REAL) (aheight - dheight);
  bdheight = (TRI_REAL) (bheight - dheight);
  cdheight = (TRI_REAL) (cheight - dheight);
...
#ifdef ANSI_DECLARATORS
TRI_REAL orient3d(struct mesh *m, struct behavior *b,
              vertex pa, vertex pb, vertex pc, vertex pd,
              TRI_REAL aheight, TRI_REAL bheight, TRI_REAL cheight, TRI_REAL dheight)
#else /* not ANSI_DECLARATORS */
TRI_REAL orient3d(m, b, pa, pb, pc, pd, aheight, bheight, cheight, dheight)
struct mesh *m;
struct behavior *b;
vertex pa;
vertex pb;
...
{
  TRI_REAL adx, bdx, cdx, ady, bdy, cdy, adheight, bdheight, cdheight;
  TRI_REAL bdxcdy, cdxbdy, cdxady, adxcdy, adxbdy, bdxady;
  TRI_REAL det;
  TRI_REAL permanent, errbound;
...
#ifdef ANSI_DECLARATORS
TRI_REAL nonregular(struct mesh *m, struct behavior *b,
                vertex pa, vertex pb, vertex pc, vertex pd)
#else /* not ANSI_DECLARATORS */
TRI_REAL nonregular(m, b, pa, pb, pc, pd)
struct mesh *m;
struct behavior *b;
vertex pa;
vertex pb;
...
void findcircumcenter(struct mesh *m, struct behavior *b,
                      ...
                      vertex circumcenter, TRI_REAL *xi, TRI_REAL *eta, int offcenter)
#else /* not ANSI_DECLARATORS */
...
{
  TRI_REAL xdo, ydo, xao, yao;
  TRI_REAL dodist, aodist, dadist;
  TRI_REAL denominator;
  TRI_REAL dx, dy, dxoff, dyoff;
...
void enqueuebadtri(struct mesh *m, struct behavior *b, struct otri *enqtri,
                   TRI_REAL minedge, vertex enqapex, vertex enqorg, vertex enqdest)
#else /* not ANSI_DECLARATORS */
...

{
  ...
  vertex joinvertex;
  TRI_REAL dxod, dyod, dxda, dyda, dxao, dyao;
  TRI_REAL dxod2, dyod2, dxda2, dyda2, dxao2, dyao2;
  TRI_REAL apexlen, orglen, destlen, minedge;
  TRI_REAL angle;
  TRI_REAL area;
  TRI_REAL dist1, dist2;
  subseg sptr;                      /* Temporary variable used by tspivot(). */
...
{
  TRI_VOID **sampleblock;
  char *firsttri;
  struct otri sampletri;
  vertex torg, tdest;
  unsigned long alignptr;
  TRI_REAL searchdist, dist;
  TRI_REAL ahead;
  ...
  while (totalsamplesleft > 0) {
    ...
    if (totalsamplesleft > 0) {
      sampleblock = (TRI_VOID **) *sampleblock;
...

{
  ...
  trifree((TRI_VOID *) m->infvertex1);  /* Deallocate the bounding box vertices. */
  trifree((TRI_VOID *) m->infvertex2);
  trifree((TRI_VOID *) m->infvertex3);
...

{
  ...
  *events = (struct event *) trimalloc(maxevents * (int) sizeof(struct event));
  ...
  for (i = 0; i < m->invertices; i++) {
    ...
    (*events)[i].eventptr = (TRI_VOID *) thisvertex;
  ...
  *freeevents = (struct event *) NULL;
  for (i = maxevents - 1; i >= m->invertices; i--) {
    (*events)[i].eventptr = (TRI_VOID *) *freeevents;
    *freeevents = *events + i;
  }
}
...
#ifdef ANSI_DECLARATORS
TRI_REAL circletop(struct mesh *m, vertex pa, vertex pb, vertex pc, TRI_REAL ccwabc)
#else /* not ANSI_DECLARATORS */
TRI_REAL circletop(m, pa, pb, pc, ccwabc)
struct mesh *m;
vertex pa;
vertex pb;
vertex pc;
...
{
  TRI_REAL xac, yac, xbc, ybc, xab, yab;
  TRI_REAL aclen2, bclen2, ablen2;
...

{
  ...
  if (eventvertex != (vertex) NULL) {
    ...
    deadevent->eventptr = (TRI_VOID *) *freeevents;
...

{
  ...
  if (checkvertex == splaytree->keydest) {
    ...
    pooldealloc(&m->splaynodes, (TRI_VOID *) splaytree);
...
struct splaynode *circletopinsert(struct mesh *m, struct behavior *b,
                                  struct splaynode *splayroot,
                                  struct otri *newkey,
                                  vertex pa, vertex pb, vertex pc, TRI_REAL topy)
#else /* not ANSI_DECLARATORS */
...
{
  TRI_REAL ccwabc;
  TRI_REAL xac, yac, xbc, ybc;
  TRI_REAL aclen2, bclen2;
  TRI_REAL searchpoint[2];
...

{
  ...
  eventheap[0]->eventptr = (TRI_VOID *) freeevents;
...
*/
    ...
    nextevent->eventptr = (TRI_VOID *) freeevents;
    ...
    if (check4events) {
      ...
      if (lefttest > 0.0) {
        ...
        newevent->ykey = circletop(m, leftvertex, midvertex, rightvertex,
                                   lefttest);
        newevent->eventptr = (TRI_VOID *) encode(lefttri);
      ...
      if (righttest > 0.0) {
        ...
        newevent->ykey = circletop(m, leftvertex, midvertex, rightvertex,
                                   righttest);
        newevent->eventptr = (TRI_VOID *) encode(farrighttri);
        eventheapinsert(eventheap, heapsize, newevent);
        heapsize++;
...
int reconstruct(struct mesh *m, struct behavior *b, int *trianglelist,
                TRI_REAL *triangleattriblist, TRI_REAL *trianglearealist,
...
struct behavior *b;
int *trianglelist;
TRI_REAL *triangleattriblist;
TRI_REAL *trianglearealist;
int elements;
int corners;
int attribs;
int *segmentlist;
  ...
  while (triangleloop.tri != (triangle *) NULL) {
    ...
    for (j = 0; j < m->eextras; j++) {
      ...
      if (*stringptr == '\0') {
        ...
        setelemattribute(triangleloop, j,
                         (TRI_REAL) strtod(stringptr, &stringptr));
      }
#endif /* not TRILIBRARY */
    }
    ...
    if (b->vararea) {
      ...
      if (*stringptr == '\0') {
        ...
        area = (TRI_REAL) strtod(stringptr, &stringptr);
      }
#endif /* not TRILIBRARY */
...
void regionplague(struct mesh *m, struct behavior *b,
                  TRI_REAL attribute, TRI_REAL area)
#else /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
void carveholes(struct mesh *m, struct behavior *b, TRI_REAL *holelist, int holes,
                TRI_REAL *regionlist, int regions)
#else /* not ANSI_DECLARATORS */
...

{
  ...
  vertex newvertex;
  enum insertvertexresult success;
  TRI_REAL segmentlength, nearestpoweroftwo;
  TRI_REAL split;
  TRI_REAL multiplier, divisor;
  ...
  if ((b->minangle > 0.0) || b->vararea || b->fixedarea || b->usertest) {
    ...
    while ((m->badtriangles.items > 0) && (m->steinerleft != 0)) {
      ...
      if (m->badsubsegs.items > 0) {
        ...
        pooldealloc(&m->badtriangles, (TRI_VOID *) badtri);
      }
    }
  }
...

{
  ...
  m->vertices.deaditemstack = (TRI_VOID *) NULL;
...

{
  ...
  for (i = 0; i < m->invertices; i++) {
    ...
    x = (TRI_REAL) strtod(stringptr, &stringptr);
    stringptr = findfield(stringptr);
    if (*stringptr == '\0') {
      printf("Error:  Vertex %d has no y coordinate.\n", b->firstnumber + i);
      triexit(1);
    }
    y = (TRI_REAL) strtod(stringptr, &stringptr);
    vertexloop[0] = x;
    vertexloop[1] = y;
    ...
    for (j = 2; j < 2 + m->nextras; j++) {
      ...
      if (*stringptr == '\0') {
        ...
        vertexloop[j] = (TRI_REAL) strtod(stringptr, &stringptr);
      }
    }
    if (nodemarkers) {
...
#ifdef ANSI_DECLARATORS
void transfernodes(struct mesh *m, struct behavior *b, TRI_REAL *pointlist,
                   TRI_REAL *pointattriblist, int *pointmarkerlist,
...
void readholes(struct mesh *m, struct behavior *b,
               FILE *polyfile, char *polyfilename, TRI_REAL **hlist, int *holes,
               TRI_REAL **rlist, int *regions)
#else /* not ANSI_DECLARATORS */
...

{
  ...
  *holes = (int) strtol(stringptr, &stringptr, 0);
  if (*holes > 0) {
    holelist = (TRI_REAL *) trimalloc(2 * *holes * (int) sizeof(TRI_REAL));
    *hlist = holelist;
    for (i = 0; i < 2 * *holes; i += 2) {
      stringptr = readline(inputline, polyfile, polyfilename);
      stringptr = findfield(stringptr);
      if (*stringptr == '\0') {
        ...
        holelist[i] = (TRI_REAL) strtod(stringptr, &stringptr);
      }
      stringptr = findfield(stringptr);
      if (*stringptr == '\0') {
        ...
        holelist[i + 1] = (TRI_REAL) strtod(stringptr, &stringptr);
      }
    }
  } else {
    *hlist = (TRI_REAL *) NULL;
  ...
  if ((b->regionattrib || b->vararea) && !b->refine) {
    ...
    *regions = (int) strtol(stringptr, &stringptr, 0);
    if (*regions > 0) {
      regionlist = (TRI_REAL *) trimalloc(4 * *regions * (int) sizeof(TRI_REAL));
      *rlist = regionlist;
      index = 0;
      for (i = 0; i < *regions; i++) {
        stringptr = readline(inputline, polyfile, polyfilename);
        ...
        if (*stringptr == '\0') {
          ...
          regionlist[index++] = (TRI_REAL) strtod(stringptr, &stringptr);
        }
        stringptr = findfield(stringptr);
        if (*stringptr == '\0') {
...
#ifdef ANSI_DECLARATORS
void writenodes(struct mesh *m, struct behavior *b, TRI_REAL **pointlist,
                TRI_REAL **pointattriblist, int **pointmarkerlist)
#else /* not ANSI_DECLARATORS */
...

{
  ...
  if (*pointlist == (TRI_REAL *) NULL) {
    *pointlist = (TRI_REAL *) trimalloc((int) (outvertices * 2 * sizeof(TRI_REAL)));
  }
  /* Allocate memory for output vertex attributes if necessary. */
  if ((m->nextras > 0) && (*pointattriblist == (TRI_REAL *) NULL)) {
    *pointattriblist = (TRI_REAL *) trimalloc((int) (outvertices * m->nextras *
                                                 sizeof(TRI_REAL)));
  }
...
void writeelements(struct mesh *m, struct behavior *b,
                   int **trianglelist, TRI_REAL **triangleattriblist)
#else /* not ANSI_DECLARATORS */
...

{
  ...
  if ((m->eextras > 0) && (*triangleattriblist == (TRI_REAL *) NULL)) {
    *triangleattriblist = (TRI_REAL *) trimalloc((int) (m->triangles.items *
                                                    m->eextras *
                                                    sizeof(TRI_REAL)));
  }
  tlist = *trianglelist;
...
void writepoly(struct mesh *m, struct behavior *b, char *polyfilename,
               TRI_REAL *holelist, int holes, TRI_REAL *regionlist, int regions,
               int argc, char **argv)
#else /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
void writevoronoi(struct mesh *m, struct behavior *b, TRI_REAL **vpointlist,
                  TRI_REAL **vpointattriblist, int **vpointmarkerlist,
                  int **vedgelist, int **vedgemarkerlist, TRI_REAL **vnormlist)
#else /* not ANSI_DECLARATORS */
...

{
  ...
  if (*vpointlist == (TRI_REAL *) NULL) {
    *vpointlist = (TRI_REAL *) trimalloc((int) (m->triangles.items * 2 *
                                            sizeof(TRI_REAL)));
  }
  /* Allocate memory for Voronoi vertex attributes if necessary. */
  if (*vpointattriblist == (TRI_REAL *) NULL) {
    *vpointattriblist = (TRI_REAL *) trimalloc((int) (m->triangles.items *
                                                  m->nextras * sizeof(TRI_REAL)));
  }
  ...
  *vedgemarkerlist = (int *) NULL;
  ...
  if (*vnormlist == (TRI_REAL *) NULL) {
    *vnormlist = (TRI_REAL *) trimalloc((int) (m->edges * 2 * sizeof(TRI_REAL)));
  }
  elist = *vedgelist;
  normlist = *vnormlist;
  coordindex = 0;
...

{
  struct otri triangleloop;
  vertex p[3];
  TRI_REAL cossquaretable[8];
  TRI_REAL ratiotable[16];
  TRI_REAL dx[3], dy[3];
  TRI_REAL edgelength[3];
  TRI_REAL dotproduct;
  TRI_REAL cossquare;
  TRI_REAL triarea;
  TRI_REAL shortest, longest;
  TRI_REAL trilongest2;
  TRI_REAL smallestarea, biggestarea;
  TRI_REAL triminaltitude2;
  TRI_REAL minaltitude;
  TRI_REAL triaspect2;
  TRI_REAL worstaspect;
  TRI_REAL smallestangle, biggestangle;
  TRI_REAL radconst, degconst;
  int angletable[18];
  int aspecttable[16];
  int aspectindex;
  int tendegree;
  ...
  degconst = 180.0 / PI;
  for (i = 0; i < 8; i++) {
    cossquaretable[i] = cos(radconst * (TRI_REAL) (i + 1));
...

{
  struct mesh m;
  struct behavior b;
  TRI_REAL *holearray;                                        /* Array of holes. */
  TRI_REAL *regionarray;   /* Array of regional attributes and area constraints. */
#ifndef TRILIBRARY
  FILE *polyfile;
#endif /* not TRILIBRARY */
#ifndef NO_TIMER
  ...
  if (b.poly || b.convex) {
    ...
    if (b.nopolywritten || b.noiterationnum) {
      ...
      if (b.poly) {
        ...
        out->holelist = (TRI_REAL *) NULL;
        out->regionlist = (TRI_REAL *) NULL;
      }
#else /* not TRILIBRARY */
...
#ifndef CDT_ONLY
  if (m.regions > 0) {
    trifree((TRI_VOID *) regionarray);
  }
#endif /* not CDT_ONLY */
  if (m.holes > 0) {
    trifree((TRI_VOID *) holearray);
  }
  if (b.geomview) {

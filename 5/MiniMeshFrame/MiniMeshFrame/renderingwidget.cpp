...
void RenderingWidget::Load_Tex(){
	...
	SW(TOOL::TEX_U, ACTIVATE);
	SW(TOOL::TEX_WLS, ACTIVATE);
	SW(TOOL::TEX_SP, ACTIVATE);
	SW(TOOL::TEX_ARAP, ACTIVATE);
...
void RenderingWidget::Tex_U(){
	START();
	Param_FB_U param;
	param.Parameterize(cur_mesh_);
	updateGL();
	END("Uniform");
}
void RenderingWidget::Tex_WLS(){
	START();
	Param_FB_WLS param;
	param.Parameterize(cur_mesh_);
	updateGL();
...
void RenderingWidget::Tex_ARAP(){
	START();
	Param_FB_SP paramSP;
	Param_UFB_ARAP paramARAP;
	paramSP.Parameterize(cur_mesh_);
	paramARAP.Parameterize(cur_mesh_);
	updateGL();
	END("ARAP");
}
...
void RenderingWidget::CheckDrawTexture(bool bv)
{
	...
	for (size_t i = 0; faces != NULL && i != faces->size(); ++i)
	{
...
void RenderingWidget::Iteration(){
	...
	for(int i=0; i<cur_mesh_->get_vertex_list()->size(); i++){
		cur_mesh_->get_vertex(i)->texCoord_[0] = mesh_vec_.back()->get_vertex(i)->texCoord_[0];
		cur_mesh_->get_vertex(i)->texCoord_[1] = mesh_vec_.back()->get_vertex(i)->texCoord_[1];
	}
#endif
	START();
	/*
	...
	*/
	MinSrfLocal::Minimize(cur_mesh_);
	updateGL();
...
void RenderingWidget::Formula(){
	...
	printf("Boundary : %d, Inner: %d\n", vertVec->size() - n, n);
	*/
	MinSrfGlobal::Minimize(cur_mesh_);
	END("Time of Formula");
...
void RenderingWidget::ClosedCurve(){
	...
	*/
	...
	for(int i=0; i<V->size(); i++){
		float x = V->at(i)->position().x();
		float y = V->at(i)->position().y();
		float z = V->at(i)->position().z();
		curve.push_back(point(x,y,z));
	}
	MinSrfCurve::Minimize(curve, cur_mesh_);
	updateGL();
...
FP RenderingWidget::method(int id){
	FP m[] = {&RenderingWidget::ReadMesh, &RenderingWidget::Load_Tex,
		&RenderingWidget::Tex_U, &RenderingWidget::Tex_WLS, &RenderingWidget::Tex_SP, &RenderingWidget::Tex_ARAP, 
		&RenderingWidget::WriteMesh, &RenderingWidget::SetBackground, 
